...
import { getWeeksInMonth, getYearOptions } from '@/lib/date';
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <FormControl variant="standard">
          <Select
            value={String(year)}
            sx={selectStyle}
            onChange={(event: SelectChangeEvent) => dispatch(setYear(Number(event.target.value)))}
          >
        ...
        <FormControl variant="standard">
          <Select
            ...
            onChange={(event: SelectChangeEvent) =>
              dispatch(setMonth(event.target.value === 'all' ? 'all' : Number(event.target.value)))
            }
          >
        ...
        <FormControl variant="standard">
          <Select
            ...
            sx={selectStyle}
            onChange={(event: SelectChangeEvent) =>
              dispatch(setWeek(event.target.value === 'all' ? 'all' : Number(event.target.value)))
            }
          >
            <MenuItem value="all">전체</MenuItem>
            {weeks.map((value) => (
              <MenuItem key={value.week} value={value.week}>{value.label}</MenuItem>
            ))}
          </Select>
        </FormControl>
      </div>
