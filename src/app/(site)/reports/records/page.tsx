...
const styles: Record<Status, string> = { normal: 'bg-white text-slate-700', late: 'bg-red-50 text-red-700', leave: 'bg-slate-100 text-slate-700', holiday: 'bg-slate-50 text-slate-400', warning: 'bg-red-50 text-red-700' };
...
export default function Page() {
  ...
  const days = useMemo(() => Array.from({ length: new Date(year, displayMonth, 0).getDate() }, (_, index) => { const day = index + 1; return { day, weekday: new Date(year, displayMonth - 1, day).getDay() }; }), [year, displayMonth]);
  ...
  const periodEmployees = useMemo(() => {
    ...
    return snapshot.employees.map<ReportEmployee>((employee) => ({
      id: employee.id,
      name: employee.name,
  ...
  const getCell = (employee: ReportEmployee, day: number): CellValue | undefined => {
